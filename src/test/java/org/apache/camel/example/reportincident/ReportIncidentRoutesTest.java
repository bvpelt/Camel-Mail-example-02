...
import nl.bsoft.camelex.ReportIncidentRoutes;
...
public class ReportIncidentRoutesTest extends ContextTestSupport {
	...
	public void testTransformMailBody() throws Exception {
		...
		// send the message (using the sendBody method that takes a parameters
		// as the input body)
		// to "direct:start" that kick-starts the route
		// the response is returned as the out object, and its also the body of
		// the response
		Object out = context.createProducerTemplate().sendBody("direct:start",
				parameters);
		// convert the response to a string using camel converters. However we
		// could also have casted it to
		// a string directly but using the type converters ensure that Camel can
		// convert it if it wasn't a string
		// in the first place. The type converters in Camel is really powerful
		// and you will later learn to
		...
		assertTrue(body
				.startsWith("Incident 123 has been reported on the 2008-07-16 by Claus Ibsen."));
	}
